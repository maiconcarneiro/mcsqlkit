...
col 5 heading "Node 5" format 999,999,999,999.99
col 6 heading "Node 6" format 999,999,999,999.99
col 7 heading "Node 7" format 999,999,999,999.99
col 8 heading "Node 8" format 999,999,999,999.99

select * from (
select name as metric_name, 
       inst_id, 
       round(value/decode(unit,'bytes',1024*1024,1),2) as metric_value
from GV$PGASTAT
where name like '%&1%'
order by 1,2
)
